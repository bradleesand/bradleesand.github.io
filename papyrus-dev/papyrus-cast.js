...
define('mathutils',['jquery', 'lodash', 'distanceutils'], function ($, _, DistanceUtils) {
...
define('pagestyle',['mathutils'], function (MathUtils) {
    return {
        BLANK: {
            disp: 'Blank',
            val: 'blank',
...
define('model/item',[
    ...
    'backbone.relational',
    'util/guid'
    ], function (_, Backbone, RelationalModel, guid) {
...
define('util/papyrus',['lodash', 'createjs', 'model/text'], function (_, createjs, Text) {
    return {
        convertPointToPapyrus: function (point, rP) {
            return {
                x: point.x - rP.x,
                y: point.y - rP.y
            };
        },
        convertPointToCanvas: function (point, rP) {
            return {
                x: point.x + rP.x,
                y: point.y + rP.y
            };
...
define('model/stroke',[
    ...
    return Item.extend({
        ...
        addPoint: function (p) {
            ...
            this.updateBounds(p);
            points.push(Papyrus.convertPointToPapyrus(p, this.get('rP')));
        },

        updateBounds: function (p) {
            // console.log('UpdateBounds: p.x:' + p.x + ' p.y:' + p.y);

            var halfWeight = this.get('weight') / 2;
            var bounds = this.get('bounds');
        ...
        recalcBounds: function () {
            ...
            len = points.length;
            for (i = 0; i < len; i++) {
                this.updateBounds(Papyrus.convertPointToCanvas(points[i], rP));
            }
        ...
        draw: function (layer) {
            ...
            if (_.isUndefined(layer)) {
                newstroke = Papyrus.createItemLayerStageObject(item);
            } else {
                newstroke = Papyrus.createStageObject(item, layer);
            }
            ...
            _.each(points, function (element) {
                convertedPoint = Papyrus.convertPointToCanvas(element, refPoint);
                // console.log(convertedPoint.x, convertedPoint.y);
                newstroke.graphics.beginStroke(item.get('color')).setStrokeStyle(
                    item.get('weight'), 'round').moveTo(prevPoint.x,
...
define('model/ellipse',[
    ...
    return Item.extend({
        ...
        draw: function (layer) {
            ...
            var item = this,
                newstroke = (_.isUndefined(layer) ?
                    Papyrus.createItemLayerStageObject(item) :
                    Papyrus.createStageObject(item, layer)),
                refPoint = item.get('rP');
...
define('papyrustools',[
    ...
    'model/ellipse',
    'model/text'
    ], function (_, createjs, MathUtils, Papyrus, PapyrusUtil, PapyrusEraser, Stroke, Ellipse, Text) {
    ////////
    // Utils
    ////////
            ...
            tools: {
                stroke: {
                    ...
                    move: function (touch, dontUpdate) {
                        ...
                        self.currentItem.addPoint(newPoint);
                    },
                    finish: function () {
                        if (self.currentItem) {
                    ...
                    finish: function () {
                        if (self.currentItem) {
                            ...
                            self.currentItem.get('points').push(
                                PapyrusUtil.convertPointToPapyrus({
                                    x: rP.x,
                                    y: rP.y
...
define('papyrus',[
    ...
    _.extend(Papyrus.prototype, {
        ...
        setPage: function (page) {
            ...
            PageStyle[background.pageStyle].draw(backgroundShape, width, height);

            // backgroundShape.cache(0, 0, width, height);
            // backgroundRect.cache(0, 0, width, height);
